local IPanel, I_f, I_t, I_tr, I_bl, I_br, title_text, I_f2

local initInspectFrame = CreateFrame("Frame")
initInspectFrame:RegisterEvent("PLAYER_LOGIN")
initInspectFrame:SetScript("OnEvent", function()
	IPanel = CreateFrame("Frame", nil, CharacterFrame)
	IPanel:SetPoint("TOPLEFT", CharacterFrame, "TOPLEFT", -50, -200)
	IPanel:SetPoint("BOTTOMRIGHT", CharacterFrame, "BOTTOMRIGHT", -200, 0)
	
	I_f = CreateFrame("Frame", "YourFrameName", IPanel)
	I_f:SetFrameStrata("HIGH")
	I_f:SetSize(400, 400)
	I_f:SetPoint("CENTER")
	I_f:Hide()

	I_t = I_f:CreateTexture(nil, "ARTWORK")
	I_t:SetTexture("Interface\\PaperDollInfoFrame\\UI-Character-General-TopLeft")
	I_t:SetPoint("TOPLEFT", CharacterFrame, "TOPLEFT", 2, -1)
	I_t:SetWidth(256)
	I_t:SetHeight(256)

	I_tr = I_f:CreateTexture(nil, "ARTWORK")
	...
	I_bl = I_f:CreateTexture(nil, "ARTWORK")
	...
	I_br = I_f:CreateTexture(nil, "ARTWORK")
	...
	title_text = I_f:CreateFontString(nil, "ARTWORK")
	...
	I_f2 = AceGUI:Create("HardcoreFrameEmpty")
...
function ShowInspectHC(_hardcore_character, other_name, version)
	...
	IPanel:SetFrameLevel(InspectFrame:GetFrameLevel() + 5)
	I_f:SetFrameLevel(InspectFrame:GetFrameLevel() + 6)
	I_f2.frame:SetFrameLevel(InspectFrame:GetFrameLevel() + 7)
