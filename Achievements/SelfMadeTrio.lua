...
local Combine = {
	35, 36, 38, 39, 40, 43, 44, 45, 47, 48, 49, 51, 52, 53, 55, 56, 57, 59, 120, 121, 127, 129, 139, 140, 147, 148, 153, 154, 1395, 1396, 2092, 2105, 2361, 2362, 2504, 2508, 2512, 2516, 2947, 3111, 3661, 6096, 6097, 6098, 6117, 6118, 6119, 6120, 6121, 6122, 6123, 6124, 6125, 6126, 6127, 6129, 6134, 6135, 6136, 6137, 6138, 6139, 6140, 6144, 12282, 20891, 20892, 20893, 20894, 20895, 20896, 20897, 20898, 20899, 20900, 20901, 20978, 20982, 23322, 23344, 23345, 23346, 23347, 23348, 23473, 23474, 23475, 23476, 23477, 23478, 23479, 24143, 24145, 24146, 25861, 28979, 49778, 50055, 50057, 34652, 34656, 34651, 34655, 34648, 34650, 34653, 34659, 34649, 34657, 34658, 38147, 38707, 52021, 41165, 52020, 41164, 10512, 15997, 23772, 10513, 8067, 8069, 8068,
}
		...
		if region and region:GetObjectType() == "FontString" then
			...
			if text and type(text) == "string" then
				if string.match(text, "...") then
					GameTooltip:Hide() 
					return true
				elseif teamduocheck and string.match(text, teamduocheck) then
					GameTooltip:Hide() 
					return true
				elseif teamtriocheck and string.match(text, teamtriocheck) then
					GameTooltip:Hide() 
					return true
				end
			end
...
self_made_trio_achievement:SetScript("OnEvent", function(self, event, ...)
	...
	if event == "PLAYER_EQUIPMENT_CHANGED" then
		...
		if GetInventoryItemID("player", 0) ~= nil then
			...
			Hardcore:Print("Equipped ammo " .. (item_name or "unknown") .. " which isn't self created.")
			self_made_trio_achievement.fail_function_executor.Fail(self_made_trio_achievement.name)
		end
	elseif event == "ITEM_UNLOCKED" and arg[2] == nil and arg[1] ~= 19 then 
			...
			if isSelfCreated(GameTooltip:GetRegions()) == false then
				if item_subtype == "Fishing Poles" or item_subtype == "Fishing Pole" then
					...
					if Start(arg[1]) == false then
						Hardcore:Print("Equipped " .. (item_name or "unknown") .. " which isn't self created.")
						self_made_trio_achievement.fail_function_executor.Fail(self_made_trio_achievement.name)
					end
				end
			end
